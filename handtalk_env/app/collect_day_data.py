"""
Script para capturar videos de los días de la semana (lunes-jueves) de la cámara para entrenar el modelo.
Permite capturar videos cortos de gestos de días en tiempo real.
"""
import cv2
...
VIDEOS_PER_CLASS = 10  # Número de videos a capturar por día
...
def collect_video_for_class(class_name):
    ...
    if existing_videos >= VIDEOS_PER_CLASS:
        print(f"[OK] Ya existen {existing_videos} videos para el día '{class_name}'")
        return existing_videos
    
    videos_needed = VIDEOS_PER_CLASS - existing_videos
    print(f"\n{'='*60}")
    print(f"Capturando videos para el día: {class_name}")
    print(f"Videos necesarios: {videos_needed}")
    print(f"{'='*60}")
    print("\nInstrucciones:")
    print(f"1. Muestra el gesto del día '{class_name}'")
    print("2. Presiona ESPACIO para iniciar la grabación (3 segundos)")
    print("3. Presiona 'q' para saltar este día")
    ...
    while captured < videos_needed:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if hand_img is not None:
                    ...
                    info_text = f"Dia: {class_name} | Videos: {captured + existing_videos}/{VIDEOS_PER_CLASS}"
                    if recording:
                        info_text += " [GRABANDO]"
                    cv2.putText(frame, info_text, (10, h-60), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0) if not recording else (0, 0, 255), 2)
                    cv2.putText(frame, "ESPACIO: Grabar | Q: Saltar dia | ESC: Salir", 
                               (10, h-30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        
        else:
            cv2.putText(frame, "No se detecta mano. Muestra el gesto del dia.", 
        ...
        if key == ord('q'):
            print(f"Saltando dia '{class_name}'")
            break
        elif key == 27:  # ESC
            ...
            recording = True
            frames_to_record = []
            print(f"  Iniciando grabacion {captured + 1}/{videos_needed}...")
...
def main():
    """Función principal"""
    print("="*60)
    print("RECOLECCION DE VIDEOS PARA DIAS (LUNES-JUEVES)")
    print("="*60)
    print(f"\nEste script te permitira capturar videos de dias desde la camara.")
    print(f"Dias disponibles: {', '.join(CLASS_NAMES)}")
    print(f"Videos por dia: {VIDEOS_PER_CLASS}")
    print(f"Duracion de cada video: {VIDEO_DURATION} segundos")
    print(f"\n¿Deseas capturar videos para todos los dias? (s/n): ", end='')
    ...
    if response == 's':
        # Capturar para todos los días
        for class_name in CLASS_NAMES:
            collect_video_for_class(class_name)
            if class_name != CLASS_NAMES[-1]:
                print(f"\nPresiona ENTER para continuar con el siguiente dia...")
                input()
    else:
        # Capturar para días específicos
        print("\nIngresa los dias que deseas capturar (separados por coma):")
        print(f"Dias disponibles: {', '.join(CLASS_NAMES)}")
        days_input = input().strip().lower()
        selected_days = [d.strip() for d in days_input.split(',') if d.strip() in CLASS_NAMES]
        
        if not selected_days:
            print("No se seleccionaron dias validos.")
            return
        
        for class_name in selected_days:
            collect_video_for_class(class_name)
            if class_name != selected_days[-1]:
                print(f"\nPresiona ENTER para continuar con el siguiente dia...")
                input()
    ...
    print("\nAhora puedes ejecutar 'train_day_model.py' para entrenar el modelo.")
