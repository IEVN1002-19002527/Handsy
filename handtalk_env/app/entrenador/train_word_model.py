"""
Script para entrenar el modelo de palabras y expresiones con videos capturados de la cámara.
...
MODEL_DIR = "model"
MODEL_PATH = os.path.join(MODEL_DIR, "word_model_camera.h5")
# Lista de palabras y expresiones (debe coincidir con collect_word_data.py)
...
def load_videos_from_directory(data_dir):
    ...
    for class_idx, class_name in enumerate(CLASS_NAMES):
        ...
        if len(video_files) == 0:
            print(f"  [ADVERTENCIA] No hay videos para la palabra '{class_name}'")
            continue
        
        print(f"  Cargando {len(video_files)} videos de la palabra '{class_name}'...")
    ...
    if len(sequences) == 0:
        raise ValueError("No se encontraron videos para entrenar. Ejecuta 'collect_word_data.py' primero.")
...
def main():
    """Función principal"""
    print("="*60)
    print("ENTRENAMIENTO DEL MODELO DE PALABRAS Y EXPRESIONES")
    print("="*60)
    ...
    if not os.path.exists(DATA_DIR):
        ...
        print("   Ejecuta 'collect_word_data.py' para capturar videos o agrega videos manualmente.")
